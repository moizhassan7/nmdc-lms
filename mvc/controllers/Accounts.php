...
*/
	...
	public function index() {
		...
		$this->data['accountss'] = $this->Accounts_m->get_order_by_accounts(array('schoolyearID' => $schoolyearID));
		$this->data["subview"] = "accounts/index";
	...
	protected function rules() {
		$rules = array(
				 array(
					'field' => 'account_name',
					'label' => $this->lang->line("accounts_accounts"),
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					$this->data["subview"] = "accounts/add";
					$this->load->view('_layout_main', $this->data);
				} else {
					$array = array(
					...
					$this->Accounts_m->insert_accounts($array);
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
			...
			if((int)$id) {
				$schoolyearID = $this->session->userdata('defaultschoolyearID');	
				$this->data['accounts'] = $this->Accounts_m->get_single_accounts(array('accountsID' => $id, 'schoolyearID' => $schoolyearID));
				if($this->data['accounts']) {
					if($_POST) {
						$rules = $this->rules();
						$this->form_validation->set_rules($rules);
						if ($this->form_validation->run() == FALSE) {
							$this->data["subview"] = "accounts/edit";
							$this->load->view('_layout_main', $this->data);
						} else {
							$array = array(
								"account_name" 		=> $this->input->post("account_name"),
							...
							$this->Accounts_m->update_accounts($array, $id);
							$this->session->set_flashdata('success', $this->lang->line('menu_success'));
							redirect(base_url("accounts/index"));
						}
					} else {
	...
	public function delete() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
			...
			if((int)$id) {
				$schoolyearID = $this->session->userdata('defaultschoolyearID');
				$accounts = $this->Accounts_m->get_single_accounts(array('accountsID' => $id, 'schoolyearID' => $schoolyearID));
				if(customCompute($accounts)) {
					$this->Accounts_m->delete_accounts($id);
					$this->session->set_flashdata('success', $this->lang->line('menu_success'));
					redirect(base_url("accounts/index"));
				} else {
					redirect(base_url("accounts/index"));
