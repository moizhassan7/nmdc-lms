...
*/
	...
	protected function rules() {
		$rules = array(
			...
			array(
				'field' => 'incomeheadID',
				'label' => $this->lang->line("income_head"),
				'rules' => 'trim|required|numeric|max_length[11]|xss_clean'
			),
			array(
				'field' => 'date',
	...
	public function add() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
			...
			$this->data['income_heads'] = $this->Incomehead_m->get_incomehead();
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$array['incomeheadID'] 	= $this->input->post('incomeheadID');
					$array['accountsID'] 	= $this->input->post('accountsID');
					$array['voucher_number'] 	= $this->input->post('voucher_number');
...
//for statement
					$arrayb['name'] 			= $this->input->post('name');
					$arrayb['in_exheadID'] 	= $this->input->post('incomeheadID');
					$arrayb['accountsID'] 	= $this->input->post('accountsID');
					$arrayb['voucher_number'] 	= $this->input->post('voucher_number');
					$arrayb['type'] 		= "Income";
					$arrayb['date'] 		= date("Y-m-d", strtotime($this->input->post("date")));
					$arrayb['in_exday'] 	= date("d", strtotime($this->input->post("date")));
					$arrayb['in_exmonth'] 	= date("m", strtotime($this->input->post("date")));
					$arrayb['in_exyear']	= date("Y", strtotime($this->input->post("date")));
					$arrayb['recamount'] 		= $this->input->post('amount');
					$arrayb['file'] 			= $this->upload_data['file']['file_name'];
					$arrayb['note'] 			= $this->input->post('note');
					$arrayb['create_date'] 	= date('Y-m-d');
					$arrayb['schoolyearID'] 	= $this->session->userdata('defaultschoolyearID');
					$arrayb['userID'] 		= $this->session->userdata('loginuserID');
					$arrayb['usertypeID'] 	= $this->session->userdata('usertypeID');
					
					$this->income_m->insert_income($array);					
					$incomeLastID = $this->db->insert_id();
					
					$arrayb['in_exID'] 	= $incomeLastID;
...
$this->Statement_m->update_statement_with_multicondition($arrayb, array('in_exID' => $incomeID, 'type' => 'Income'));
