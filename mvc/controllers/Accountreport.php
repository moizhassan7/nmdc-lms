...
*/
	public function __construct() {
		...
		$this->lang->load('accountreport', $language);
	}
	...
	protected function rules() {
		$rules = array(
			array(
				'field' => 'accountsID',
				'label' => $this->lang->line("accountsID"),
				'rules' => 'trim|xss_clean'
			),
			array(
				'field' => 'schoolyearID',
				'label' => $this->lang->line("accountreport_academicyear"),
				'rules' => 'trim|required|xss_clean'
			),
			array(
				'field' => 'fromdate',
				'label' => $this->lang->line("accountreport_fromdate"),
				'rules' => 'trim|xss_clean|callback_date_valid|callback_unique_date'
			),
			array(
				'field' => 'todate',
				'label' => $this->lang->line("accountreport_todate"),
				'rules' => 'trim|xss_clean|callback_date_valid|callback_unique_date'
			),
		);
		return $rules;
	...
	protected function send_pdf_to_mail_rules() {
		$rules = array(
			...
			array(
				'field' => 'to',
				'label' => $this->lang->line("accountreport_to"),
				'rules' => 'trim|required|xss_clean|valid_email'
			),
			array(
				'field' => 'subject',
				'label' => $this->lang->line("accountreport_subject"),
				'rules' => 'trim|required|xss_clean'
			),
			array(
				'field' => 'message',
				'label' => $this->lang->line("accountreport_message"),
				'rules' => 'trim|xss_clean'
			),
		);
		return $rules;
 	...
 	public function index() {
		...
		$this->data["subview"] = "accountreport/AccountReportView";
	...
	public function queryArray($array) {
		...
		if(!empty($array['accountsID'])) {
			$retArray['accountsID']= $array['accountsID'];
		}
...
$this->data['schoolyearName'] = customCompute($schoolyear) ? $schoolyear->schoolyear : $this->lang->line('accountreport_all_accademic_year');

$accountz = $this->Accounts_m->get_single_accounts(array('accountsID'=>$array['accountsID']));
$this->data['accountName'] = customCompute($accountz) ? $accountz->account_name : $this->lang->line('accountreport_all_Accounts');
...
$totalliabilityz = $this->liability_m->get_liability_order_by_with_date_schoolyear($array);
...
$recviatransfer = $this->statement_m->get_statement_order_by_with_date_schoolyearz($array);

$sntviatransfer = $this->statement_m->get_statement_order_by_with_date_schoolyearzz($array);
...
//for income via fee
$payments = $this->payment_m->get_payment_with_income_schoolyear($array);
					$totalcollectionincome = 0;
					if(customCompute($payments)) {
						foreach ($payments as $payment) {
							if($payment->paymentamount != null) {
								$totalcollectionincome += $payment->paymentamount;
							}
						}
					}
//for liability via fee	
$payments = $this->payment_m->get_payment_with_liability_schoolyear($array);
					$totalcollectionliability = 0;
					if(customCompute($payments)) {
						foreach ($payments as $payment) {
							if($payment->paymentamount != null) {
								$totalcollectionliability += $payment->paymentamount;
							}
						}
					}
						
					$this->data['totalincome'] = $totalincome->amount;
					$this->data['totalliabilityz'] = $totalliabilityz->amount;
		...
		$this->data['totalsalarypayment'] = $totalsalarypayment->payment_amount;
					...
					$this->data['totalcollectionincome'] = $totalcollectionincome;
		$this->data['totalcollectionliability'] = $totalcollectionliability;
		
		$this->data['recviatransfer'] = $recviatransfer->recamount;
		$this->data['sntviatransfer'] = $sntviatransfer->sntamount;

					$this->data['fromdate'] = $this->input->post('fromdate');
					$this->data['todate'] = $this->input->post('todate');
				$this->data['accountsID'] = $this->input->post('accountsID');
				$this->data['schoolyearID'] = $this->input->post('schoolyearID');

$retArray['render'] = $this->load->view('accountreport/AccountReport', $this->data, true);
	...
	public function pdf() {
		if(permissionChecker('accountledgerreport')) {
			$accountsID = htmlentities(escapeString($this->uri->segment(3)));
			...
			if(((int)$schoolyearID || ($schoolyearID ==0)) && ((int)$fromdate || ($fromdate == null)) && ((int)$todate || $todate == null)) {
				...
				$postArray['accountsID'] = $accountsID;
...
//for liability via fee	
		...
		$this->data['sntviatransfer'] = $sntviatransfer->sntamount;
				...
				$this->reportPDF('accountreport.css',$this->data,'accountreport/AccountReportPDF');
			} else {
		...
		if(permissionChecker('accountledgerreport')) {
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->data['schoolyearID']   = $this->input->post('schoolyearID');
					$this->data['accountsID']   = $this->input->post('accountsID');
					$this->reportSendToMail('accountreport.css', $this->data, 'accountreport/AccountReportPDF', $to, $subject, $message);
					$retArray['status'] = TRUE;
				    echo json_encode($retArray);
				    exit;
				}
			} else {
				$retArray['message'] = $this->lang->line('accountreport_permissionmethod');
				echo json_encode($retArray);
				exit;
			}
		} else {
			$retArray['message'] = $this->lang->line('accountreport_permission');
			echo json_encode($retArray);
			exit;
		}
	}
