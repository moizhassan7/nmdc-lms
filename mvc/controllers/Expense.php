...
$this->data['expenses'] = $this->expense_m->get_order_by_expense(array('schoolyearID' => $schoolyearID));
	...
	protected function rules() {
		$rules = array(
				...
				array(
				'field' => 'expenseheadID',
				'label' => $this->lang->line("expense_head"),
					'rules' => 'trim|required|numeric|max_length[11]|xss_clean'
			    ),
				array(
					'field' => 'date',
	...
	public function add() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
			...
			$this->data['expense_heads'] = $this->Expensehead_m->get_expensehead();
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$array = array(
						...
						"expenseheadID" => $this->input->post("expenseheadID"),
					...
					$expenseLastID = $this->db->insert_id();
					
					$arrayb = array(
						"name" 			=> $this->input->post("expense"),
						"in_exID" 		=> $expenseLastID,
						"in_exheadID" 	=> $this->input->post("expenseheadID"),
						"accountsID" 	=> $this->input->post("accountsID"),
						"voucher_number" 	=> $this->input->post("voucher_number"),
						"type" 			=> "Expense",
...
$this->Statement_m->update_statement_with_multicondition($arrayb, array('in_exID' => $id, 'type' => 'Expense'));
							$this->session->set_flashdata('success', $this->lang->line('menu_success'));
							redirect(base_url("expense/index"));
						}
					} else {
