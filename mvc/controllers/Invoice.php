    ...
    protected function rules($statusID = 0) {
        ...
        if($statusID != 0) {
			...
			array(
                ...
                'rules' => 'trim|required|xss_clean|max_length[50]|callback_unique_paymentmethodID'
            ),
            array(
			...
			array(
                'field' => 'Description',
                'label' => $this->lang->line("invoice_Description"),
    ...
    public function paidinvoice() {
        ...
        if($usertypeID == 3) {
            ...
            if(customCompute($student)) {
                ...
                $this->data["subview"] = "invoice/paidinvoice";
                $this->load->view('_layout_main', $this->data);
            } else {
                $this->data["subview"] = "error";
                $this->load->view('_layout_main', $this->data);
    ...
    public function partially_paid() {
        ...
        if($usertypeID == 3) {
            ...
            if(customCompute($student)) {
                ...
                $this->data["subview"] = "invoice/partially_paid";
                $this->load->view('_layout_main', $this->data);
            } else {
                $this->data["subview"] = "error";
                $this->load->view('_layout_main', $this->data);
...
$this->data["student"] = $this->student_m->get_single_student(array('studentID' => $this->data["maininvoice"]->maininvoicestudentID));
    ...
    protected function payment_rules($invoices) {
        $rules = array(
            array(
                'field' => 'paymentmethodID',
                'label' => $this->lang->line("invoice_paymentmethod"),
                'rules' => 'trim|required|xss_clean|max_length[50]|callback_check_valid_paymentmethod|callback_unique_paymentmethod'),
			array(
                'field' => 'voucher_number',
                'label' => $this->lang->line("voucher_number"),
                'rules' => 'trim|required|xss_clean|max_length[20]'
    ...
    public function check_valid_paymentmethod() {
        $usertypeID = $this->session->userdata('usertypeID');
        $paymentMethod = ['Cash', 'Cheque', 'Bank Deposit', 'Bank Draft', 'Online Payment'];
    ...
    public function unique_paymentmethod() {
        if($this->input->post('paymentmethodID') === '0') {
            ...
            if($this->input->post('paymentmethodID') == 'Cash' || $this->input->post('paymentmethodID') == 'Cheque' || $this->input->post('paymentmethodID') == 'Bank Deposit' || $this->input->post('paymentmethodID') == 'Bank Draft' || $this->input->post('paymentmethodID') == 'Online Payment') {
                return TRUE;
...
if($this->input->post('paymentmethodID') == 'Cash' || $this->input->post('paymentmethodID') == 'Cheque' || $this->input->post('paymentmethodID') == 'Bank Deposit' || $this->input->post('paymentmethodID') == 'Bank Draft' || $this->input->post('paymentmethodID') == 'Online Payment') {
      ...
      'invoicename' => $this->data['student']->srroll .'-'. $this->data['student']->srname,
                                      'invoicedescription' => $this->input->post('Description'),
...
'tr_id' => $tr_date.$schoolyearID.$invoice->studentID.$invoice->invoiceID,
...
$panel_payment_type='panel_payment_type' => (($this->input->post('panel_payment_type_'.$invoice->invoiceID) == '') ? NULL : $this->input->post('panel_payment_type_'.$invoice->invoiceID)),
...
$panel_payment_typesz =(($this->input->post('panel_payment_type_'.$invoice->invoiceID) == '') ? NULL : $this->input->post('panel_payment_type_'.$invoice->invoiceID));

if((($this->input->post('paidamount_'.$invoice->invoiceID) == '') ? NULL : $this->input->post('paidamount_'.$invoice->invoiceID)) != ''){
...
$in_exnames="Fee via Invoice";
					...
					$arrayincome['accountsID'] 	= $this->input->post('accountsID');
					$arrayincome['invoiceID'] 	= $invoice->invoiceID;
					$arrayincome['studentID'] 	= $invoice->studentID;
$arrayincome['tr_id'] 	= $tr_date.$schoolyearID.$invoice->studentID.$invoice->invoiceID;
$arrayincome['voucher_number'] 	= $this->input->post('voucher_number');
					$arrayincome['date'] 			= date('Y-m-d');
					$arrayincome['incomeday'] 	= date("d");
					$arrayincome['incomemonth'] 	= date("m");
					$arrayincome['incomeyear']	= date("Y");
					$arrayincome['amount'] 		= (($this->input->post('paidamount_'.$invoice->invoiceID) == '') ? NULL : $this->input->post('paidamount_'.$invoice->invoiceID));
					$arrayincome['note'] 			= $this->input->post('Description');
					$arrayincome['create_date'] 	= date('Y-m-d');
					$arrayincome['schoolyearID'] 	= $this->session->userdata('defaultschoolyearID');
...
"voucher_number" 			=> $this->input->post("voucher_number"),
						"liabilityheadID" 		=> 1,
						"amount" 		=> (($this->input->post('paidamount_'.$invoice->invoiceID) == '') ? NULL : $this->input->post('paidamount_'.$invoice->invoiceID)),
...
//for inserting into statement
					...
					$arraystatement['accountsID'] 	= $this->input->post('accountsID');
					$arraystatement['invoiceID'] 	= $invoice->invoiceID;
					$arraystatement['studentID'] 	= $invoice->studentID;
$arraystatement['tr_id'] 	= $tr_date.$schoolyearID.$invoice->studentID.$invoice->invoiceID;
$arraystatement['voucher_number'] 			= $this->input->post('voucher_number');
					...
					$arraystatement['recamount'] 		= (($this->input->post('paidamount_'.$invoice->invoiceID) == '') ? NULL : $this->input->post('paidamount_'.$invoice->invoiceID));
$arraystatement['note'] 			= $this->input->post('Description');
					$arraystatement['create_date'] 	= date('Y-m-d');
				$arraystatement['schoolyearID'] 	= $this->session->userdata('defaultschoolyearID');
...
$in_exname=2; // for fine
					...
					$arrayincome['invoiceID'] 	= $invoice->invoiceID;
					$arrayincome['studentID'] 	= $invoice->studentID;
$arrayincome['tr_id'] 	= $tr_datez.$schoolyearID.$invoice->studentID.$invoice->invoiceID;
$arrayincome['voucher_number'] 	= $this->input->post('voucher_number');
					$arrayincome['date'] 			= date('Y-m-d');
					$arrayincome['incomeday'] 	= date("d");
					$arrayincome['incomemonth'] 	= date("m");
					$arrayincome['incomeyear']	= date("Y");
					$arrayincome['amount'] 		= (($this->input->post('fine_'.$invoice->invoiceID) == '') ? 0 : $this->input->post('fine_'.$invoice->invoiceID));
				$arrayincome['note'] 			= $this->input->post('Description');
					$arrayincome['create_date'] 	= date('Y-m-d');
					$arrayincome['schoolyearID'] 	= $this->session->userdata('defaultschoolyearID');
					$arrayincome['userID'] 		= $this->session->userdata('loginuserID');
...
$in_extype="Fine Income";
					...
					$arraystatement['invoiceID'] 	= $invoice->invoiceID;
					$arraystatement['studentID'] 	= $invoice->studentID;
$arraystatement['tr_id'] 	= $tr_datez.$schoolyearID.$invoice->studentID.$invoice->invoiceID;
$arraystatement['voucher_number'] 		= $this->input->post('voucher_number');
					$arraystatement['type'] 		= $in_extype;
					$arraystatement['date'] 		= date("Y-m-d");
					$arraystatement['in_exday'] 	= date("d");
					$arraystatement['in_exmonth'] 	= date("m");
					$arraystatement['in_exyear']	= date("Y");
					$arraystatement['recamount'] 	= (($this->input->post('fine_'.$invoice->invoiceID) == '') ? 0 : $this->input->post('fine_'.$invoice->invoiceID));
			$arraystatement['note'] 		= $this->input->post('Description');
					$arraystatement['create_date'] 	= date('Y-m-d');
				$arraystatement['schoolyearID'] 	= $this->session->userdata('defaultschoolyearID');
					$arraystatement['userID'] 		= $this->session->userdata('loginuserID');
...
$this->globalpayment_m->update_globalpayment(array('note' => $this->input->post('note')), $globalLastID);
$this->globalpayment_m->update_globalpayment(array('invoicedescription' => $this->input->post('Description')), $globalLastID);
...
$panel_payment_type = pluck($this->feetypes_m->get_feetypes(), 'panel_payment_type', 'feetypesID');
                        $feetypeitems = json_decode($this->input->post('feetypeitems'));
                        $schoolyearID = $this->session->userdata('defaultschoolyearID');
...
'maininvoicestatus' => (($this->input->post('statusID') !== '0') ? (((float)$this->input->post('totalsubtotal') == (float)0) ? 2 :  (((float)$this->input->post('totalpaidamount') > (float)0) ? ((float) $this->input->post('totalsubtotal') == (float) $this->input->post('totalpaidamount') ? 2 : 1) : 0)) : 0),
                           ...
                           'maininvoicedate' => date("Y-m-d", strtotime($this->input->post("date"))),
                               ...
                               'invoicename' => $getstudent->srroll.'-'.$getstudent->srname,
                                        'invoicedescription' => $this->input->post('Description'),
                                        'note' => $this->input->post('note'),
                                        'paymentyear' => date('Y'),
                                        'schoolyearID' => $schoolyearID,
...
'panel_payment_type' => isset($panel_payment_type[$feetypeitem->feetypeID]) ? $panel_payment_type[$feetypeitem->feetypeID] : '',
...
$tr_datez=date('dmYHis').rand(11111,99999);
							 ...
							 $paymentamount='paymentamount' 	=> isset($paymentHistoryArray[$k]['paymentamount']) ? (($paymentHistoryArray[$k]['paymentamount'] == "") ? NULL : $paymentHistoryArray[$k]['paymentamount'] ) : 0,
			
'panel_payment_type' => $invoiceArray[$k]['panel_payment_type'],
'accountsID' 		 => $this->input->post('accountsID'),
								
'tr_id' => $tr_date.$invoiceArray[$k]['schoolyearID'].$invoiceArray[$k]['studentID'].$i,
'voucher_number' 		 => $this->input->post('voucher_number'),
...
$panel_payment_typesz =$invoiceArray[$k]['panel_payment_type'];
...
$in_exnames="Fee via Invoice";
					...
					$arrayincome['incomeheadID'] 	= 1;
					$arrayincome['accountsID'] 		= $this->input->post('accountsID');
					$arrayincome['invoiceID'] 		=  $i;
					$arrayincome['studentID'] 		= $invoiceArray[$k]['studentID'];
					
$arrayincome['tr_id'] = $tr_date.$invoiceArray[$k]['schoolyearID'].$invoiceArray[$k]['studentID'].$i;
$arrayincome['voucher_number'] 			= $this->input->post('voucher_number');
					$arrayincome['date'] 			= date('Y-m-d');
					$arrayincome['incomeday'] 		= date("d");
					$arrayincome['incomemonth'] 	= date("m");
					$arrayincome['incomeyear']		= date("Y");
					$arrayincome['amount'] 			= $paymentamount;
				$arrayincome['note'] 			= $this->input->post('Description');
					$arrayincome['create_date'] 	= date('Y-m-d');
					$arrayincome['schoolyearID'] 	= $this->session->userdata('defaultschoolyearID');
		$arrayincome['userID'] 		= $this->session->userdata('loginuserID');
...
//for inserting into statement
					...
					$arraystatement['in_exheadID'] 	= 1;
					$arraystatement['accountsID'] 	= $this->input->post('accountsID');
					$arraystatement['invoiceID'] 	=  $i;
					$arraystatement['studentID'] 	= $invoiceArray[$k]['studentID'];
					
$arraystatement['tr_id'] = $tr_date.$invoiceArray[$k]['schoolyearID'].$invoiceArray[$k]['studentID'].$i;
					$arraystatement['type'] 		= $in_extype;
					$arraystatement['date'] 		= date("Y-m-d");
					$arraystatement['in_exday'] 	= date("d");
					$arraystatement['in_exmonth'] 	= date("m");
					$arraystatement['in_exyear']	= date("Y");
					$arraystatement['recamount'] 	= $paymentamount;
$arraystatement['voucher_number'] 		= $this->input->post('voucher_number');
				$arraystatement['note'] 		= $this->input->post('Description');
					$arraystatement['create_date'] 	= date('Y-m-d');
				$arraystatement['schoolyearID'] 	= $this->session->userdata('defaultschoolyearID');
					...
					$this->Statement_m->insert_statement($arraystatement);
					$statementLastID = $this->db->insert_id();																										
}
												$k++;
                                            }
                                        }
                ...
                if(customCompute($paymentlists)) {
                    ...
                    foreach ($paymentlists as $key => $paymentlist) {
                        ...
                        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('usertypeID') == 5)) {
                            if(($this->lang->line('Cash') == $paymentlist['paymenttype']) || ($this->lang->line('Cheque') == $paymentlist['paymenttype']) || ('Cash' == $paymentlist['paymenttype']) || ('Cheque' == $paymentlist['paymenttype']) || ('Cheque' == $paymentlist['paymenttype']) || ('Bank Deposit' == $paymentlist['paymenttype']) || ('Bank Draft' == $paymentlist['paymenttype']) || ('Online Payment' == $paymentlist['paymenttype'])) {
            ...
            if(permissionChecker('invoice_delete')) {
                if((int)$globalpaymentID && (int)$maininvoiceID) {
                    ...
                    if(customCompute($globalpayment)) {
                        ...
                        $excType = TRUE;
                        foreach ($payments as $payment) {
                            if(($this->lang->line('Cash') == $payment->paymenttype) || ($this->lang->line('Cheque') == $payment->paymenttype) || ('Cash' == $payment->paymenttype) || ('Cheque' == $payment->paymenttype) || ('Bank Deposit' == $payment->paymenttype) || ('Bank Draft' == $payment->paymenttype) || ('Online Payment' == $payment->paymenttype)) {
