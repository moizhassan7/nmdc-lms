...
class Expensehead_m extends MY_Model {

	protected $_table_name = 'expense_heads';
	protected $_primary_key = 'expenseheadID';
	protected $_primary_filter = 'intval';
	protected $_order_by = "expenseheadID desc";

	function __construct() {
		parent::__construct();
	}

	function get_expensehead($array=NULL, $signal=FALSE) {
		$query = parent::get($array, $signal);
		return $query;
	}

	function get_single_expensehead($array=NULL) {
		$query = parent::get_single($array);
		return $query;
	}

	function get_order_by_expensehead($array=NULL) {
		$query = parent::get_order_by($array);
		return $query;
	}

	function insert_expensehead($array) {
		$error = parent::insert($array);
		return TRUE;
	}
	
	function update_expensehead($data, $id = NULL) {
		parent::update($data, $id);
		return $id;
	}

	public function delete_expensehead($id){
		parent::delete($id);
	}

	public function user_expensehead($table, $username, $email){
		$query = $this->db->get_where($table, array("username" => $username, "email" => $email));
		return $query->row();
	}

	public function get_expensehead_order_by_date($array) {
		$this->db->select('*');
...
/*
	public function get_expensehead_order_with_date_schoolyear($array) {
    ...
    /* define for 4.4
    public function get_expensehead_with_user($array = ['expense.schoolyearID' => 1])
    {
