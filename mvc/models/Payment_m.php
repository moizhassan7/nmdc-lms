...
//for fine
	public function get_payment_with_fine_schoolyear($array) {
		...
		if(isset($array['accountsID'])) {
			$this->db->where('payment.accountsID',$array['accountsID']);
		}
		$query = $this->db->get();
		return $query->result();
	}
...
//for income via fee
	public function get_payment_with_income_schoolyear($array) {
		$this->db->select('*');
		$this->db->from($this->_table_name);
		$this->db->where('payment.panel_payment_type =','Income');
		if(isset($array['fromdate']) && isset($array['todate'])) {
			$this->db->where('payment.paymentdate >=',$array['fromdate']);
			$this->db->where('payment.paymentdate <=',$array['todate']);
		} 
...
//for liability via fee	
	public function get_payment_with_liability_schoolyear($array) {
		$this->db->select('*');
		$this->db->from($this->_table_name);
		$this->db->where('payment.panel_payment_type =','Liability');
		if(isset($array['fromdate']) && isset($array['todate'])) {
			$this->db->where('payment.paymentdate >=',$array['fromdate']);
			$this->db->where('payment.paymentdate <=',$array['todate']);
		} 
